...
function vtu_post(string $endpoint, array $payload): array {
    ...
    if ($body === false) {
        return ['ok'=>false,'http_code'=>$code?:0,'error'=>$err?:'cURL error','raw'=>null,'data'=>null];
    }

    $decoded = json_decode($body, true);
    if (!is_array($decoded)) {
        return ['ok'=>false,'http_code'=>$code,'error'=>'Invalid JSON from provider','raw'=>$body,'data'=>null];
    }
